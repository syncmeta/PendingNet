...
**每次跑都会把服务端二进制对齐到该用的版本**：和已装的那份比 sha256，一样就一个字节都不动，
不一样就备份、替换、重启 `pendingnet-server.service`（起不来会自动回滚到升级前那份）。
代理节点那两个服务不受影响。这一步不能省——机器上留着旧版服务端时，它的 `pair create`
吐的是老格式的 `.pdn` JSON 而不是 `pendingnet://` 链接。要沿用已装的那份加 `--no-upgrade`，
